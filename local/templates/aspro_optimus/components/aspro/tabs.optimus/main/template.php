			...
			<div class="top_blocks">
				<ul class="tabs">
					...
					foreach($arResult["TABS"] as $code=>$title):?>
						<? if ($i != 5) { ?> <li data-code="<?=$code?>" <?=($i==1 ? "class='cur'" : "")?>><span><?=$title;?></span> <? } ?>
						<?=($i==1 ? '<a class="link-all-tabs" href="/khity-prodazh.php">(Посмотреть все)</a>' : "")?>
						<?=($i==2 ? '<a class="link-all-tabs" href="/my-sovetuem.php">(Посмотреть все)</a>' : "")?>
						<?=($i==3 ? '<a class="link-all-tabs" href="/novinki.php">(Посмотреть все)</a>' : "")?>
						<?=($i==4 ? '<a class="link-all-tabs" href="/aktsii.php">(Посмотреть все)</a>' : "")?>
						</li>
						<?$i++;?>
					<?endforeach;?>
					<li class="stretch"></li>
